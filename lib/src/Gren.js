...
class Gren {
    ...
    async _getMergedPullRequests(since, releaseRanges, page = 1, limit = 1000) {
        ...
        const totalPages = this._getLastPage(link);
        const resultPrs = [];
        const filteredPrs = prs.filter(pr => !pr.labels.map(l => l.name).some(name => this.options.ignoreLabels.includes(name)));
        for (const pr of filteredPrs) {
            if (!pr.merge_commit_sha || !pr.merged_at) continue;
        ...
        if (prs.length > 0 && totalPages && +page < totalPages) {
            return this._getMergedPullRequests(since, releaseRanges, page + 1).then(prsResults => prsResults.concat(resultPrs));
        }
