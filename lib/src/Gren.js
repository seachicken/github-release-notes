...
class Gren {
     ...
     */
    async _getMergedPullRequests(since, releaseRanges, page = 1, limit = 1000) {
        ...
        const totalPages = this._getLastPage(link);
        const latestTag = releaseRanges[0][0].name;
        const filterPrs = [];
        for (const pr of prs) {
            const containedTag = await this._git(`tag --contains ${pr.merge_commit_sha}`)
                .catch(() => {});
            if (!containedTag) continue;

            const firstMergedTag = containedTag.split('\n')
                .filter(t => !this.options.ignoreTagsWith.includes(t))[0];
            if (firstMergedTag === latestTag) {
                filterPrs.push(pr);
            }
        }
        if (prs.length > 0 && totalPages && +page < totalPages) {
            return this._getMergedPullRequests(since, releaseRanges, page + 1).then(prsResults => prsResults.concat(filterPrs));
        }
    ...
    async _getPullRequestsBlocks(releaseRanges) {
        ...
        const prs = await this._getMergedPullRequests(since, releaseRanges);
